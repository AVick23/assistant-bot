"""
🎯 Прогресс Бот v2.0
Переработанный UX в стиле Apple: минимализм, интуитивность, забота о пользователе.

Исправлены все критические ошибки:
- Импорт traceback в начале файла
- Правильное получение вопроса в feedback_callback  
...
import warnings
import logging
import traceback  # ✅ ИСПРАВЛЕНО: Импорт в начале файла
...
async def consultation_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Проверка на дубликаты (в течение последних 24 часов)
...
# ============================================================
# 💚 ОБРАТНАЯ СВЯЗЬ (ИСПРАВЛЕНО)
# ============================================================

async def feedback_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    ✅ ИСПРАВЛЕНО: Правильная обработка лайков/дизлайков
    - Проверка границ индекса
    - Правильное получение вопроса
    ...
    # ✅ Проверка валидности kb_index и границ
    if not kb_index:
    ...
    if not kb_index.is_valid_index(idx):
        logger.error(f"Index {idx} out of bounds (max: {len(kb_index.items) - 1})")
        await query.answer("Ответ не найден", show_alert=True)
        return
    
    answer = kb_index.items[idx]["context"]
    
    # ✅ ИСПРАВЛЕНО: Правильное получение вопроса
    question = get_question_for_answer(user.id, idx)
    
    # Fallback на последний вопрос из истории
    if question == "???":
    ...
    feedback_list.append({
        "type": fb_type,
        "question": question,
        "answer": answer[:200] + "..." if len(answer) > 200 else answer,  # Обрезаем длинные ответы
        "user_id": user.id,
    ...
    # 🎨 Визуальная обратная связь (Apple-style)
    if fb_type == "like":
